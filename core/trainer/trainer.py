"""
YOU ARE HERE, FIGURE OUT WHY MISREPORTS NEVER ABOVE 1 and WHY setting high misreport is bad
this is acutally a bit wrong, misreports shouldn't have all 1000000, wouldn't ever happen

how do u get misreports from x_mis? for each supplier i (misreports[i]) do they change only their costs, or others too?
actually now i think uve figured that stuff out, now ur task is to see if the training of these adv things is legit, is it really only one gradient update
actually it is a lotta gradient updates, problem was capping the misreport stuff to 1 (or was it a problem? Because costs are drawn from 0-1 dist)

ok so now u've changed the final loss formula a bit and u've aded a lot more epochs to trainig adv stuff, ur shit is running on tmux, check it
after it's finished and u contemplate it, run again iwth the new final loss formla and acutally start writing stuff in ur paper
also wuts the point of having the regret budget gradually annealing it ot desired, u have such a big regret in the beginning so it's kinda weird, look into that
mayb also change up rgt_lr and malso make misreport num epochs a bit bigger
also make it so just tryingto minimize regret and see what happens
also maybe do some regret percent erro thing as loss term instead of just regret?? to make sure small costs get same weight too? im not sure

ok so looks like u do need the *cost shit in ur update of w_rgt

also u need to figure out when u cap regret misreports between 0 and 1 since that's ur distribution
finish ipmlementing vickrey_regret
"""
import json
import time
...
class Trainer(object):
    ...
    def train_op(self, x, iteration):
        ...
        if (iteration % self.config.train.print_iter) == 0:
            self.print_mechanism(pay_true, alloc_true, x, 1)

        rgt = self.compute_regret(x, alloc_true, pay_true, x_mis, a_mis, p_mis).sum()

        # Revenue
        cost = self.compute_rev(pay_true)

        # Dual gradient decent
        self.w_rgt = max(
            0,
            self.w_rgt
            + self.config.train.rgt_lr * ((rgt).detach().log().item() - np.log(self.rgt_target)), #CHANGED
        )

        final_loss = cost + self.w_rgt * rgt
    ...
    def get_misreports_grad(self, x):
        ...
        #for i in range(512):
        #    y[0][0][i][0][0]=10000000
        #    y[1][0][i][1][0]=10000000
        ...
        #"""
            ...
            #print(y.size())
            #print("----")
            #print(misreports.size())
            #print(adv_mask.requires_grad)
            #print(torch.max(adv_mask))
            #print(torch.max(adv))
            #print(adv.requires_grad)
            #print(torch.max(adv))
            #misreports  = torch.full(misreports.size(), 1000000).float().to(self.device) #CHANGED
            #"""
    ...
    def train_epoch(self, iteration):
        ...
        net_loss, train_cost, train_regret = self.train_op(x, iteration)
        ...
        if (iteration % self.config.train.print_iter) == 0:
            ...
            print(
                "Train cost: {},   regret: {},   net loss: {} , w: {}".format(
                    round(float(train_cost), 5),
                    round(float(train_regret), 5),
                    round(float(net_loss), 5),
                    round(self.w_rgt, 4),
                )
            )
            self.writer.add_scalar("Train/cost", train_cost, iteration / 1000)
...
class DistillationTrainer(Trainer):
    ...
    def train_epoch(self, iteration):
        ...
        # Make a step for net weights updating
        net_loss, train_cost, target_cost = self.train_op(x)
        ...
        if (iteration % self.config.train.print_iter) == 0:
            ...
            print(
                "Train revenue: {}, target revenue: {}, net loss: {}".format(
                    round(float(train_cost), 5),
                    round(float(target_cost), 5),
                    round(float(net_loss), 5),
                )
            )
            self.writer.add_scalar("Train/revenue", train_cost, iteration / 1000)
            self.writer.add_scalar("Train/revenue_target", target_cost, iteration / 1000)
            self.writer.add_scalar("Train/loss", net_loss, iteration / 1000)
...
class TrainerCrossVal(object):
    ...
    def mis_step(self, x, c):
        ...
        # Run net for misreports
        a_mis, p_mis = self.net(self.create_input(misreports))
