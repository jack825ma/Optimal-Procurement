...
def vickery(batch, language="additive"):
    """
    :param batch: tensor with bids (supplier costs) in auctions shaped as (batch_size, n_agents, num_items)
    item-wise vickery auction
    """
    t = torch.zeros(batch.shape[0], batch.shape[1])
    for i in range(batch.shape[2]):
        t += -VCG(-batch[:,:,i].unsqueeze(2), language)
    return t

def VCG(batch, language="additive"):
    """
    VCG efficient and truthful mechanism.
    :param batch: tensor with bids (valuations) in auctions shaped as (batch_size, n_agents, 1)
...
if __name__ == "__main__":
    ...
    prices = vickery(batch, 'marginal').numpy()
